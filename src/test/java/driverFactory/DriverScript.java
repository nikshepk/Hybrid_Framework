			...
			if(xl.getCellData(TestCases, i, 2).equalsIgnoreCase("Y"))
			{
				...
				for(int j=1;j<=xl.rowCount(TCModule);j++)
				{
					...
					try {
						...
						if(Object_Type.equalsIgnoreCase("capurePurcNum")) {
							FunctionLibrary.capurePurcNum(Locator_Type, Locator_Value);
						}
						if(Object_Type.equalsIgnoreCase("CalenderAction")) {
							FunctionLibrary.CalenderAction();
						}
						if(Object_Type.equalsIgnoreCase("capureTotBal")) {
							FunctionLibrary.capureTotBal(Locator_Type, Locator_Value);
						}
